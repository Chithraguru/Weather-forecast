...
fetch('https://api.openweathermap.org/data/2.5/forecast?q='+newName.value+'&appid=fa388544a3e0abd01c50bd9e444707b6')
...
.then(data => {
    ...
    for(i = 0; i<5; i++){
        document.getElementById("day" + (i+1) + "Speed").innerHTML = "WindSpeed :" + Number(data.list[i].wind.speed).toFixed(0) + "mph";
    }
    //------------------------------------------------------------
...
function DefaultScreen(){
    document.getElementById("cityInput").defaultValue = "New Delhi";
    GetInfo();
}
